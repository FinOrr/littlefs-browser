...
def _bundle_dir() -> str:
    ...
    When frozen by PyInstaller this is sys._MEIPASS; otherwise the directory
    that contains this source file.
    """
...
def get_lfs_binary() -> str:
    """Return the path to the lfs FUSE binary.

    Prefers the copy bundled alongside the executable; falls back to whatever
    is on PATH so the regular development workflow still works.
    """
    bundled = os.path.join(_bundle_dir(), 'lfs')
...
# Flask app
app = Flask(__name__, static_folder=os.path.join(_bundle_dir(), 'frontend', 'dist'), static_url_path='')
CORS(app)
...
def api_delete():
    """Delete a file or directory from the mounted device"""
    device_path = request.args.get('device')
    file_path = request.args.get('path')

    if not device_path or not file_path:
        return jsonify({'success': False, 'error': 'Missing device or path'}), 400

    if device_path not in active_mounts:
        return jsonify({'success': False, 'error': ERROR_DEVICE_NOT_MOUNTED}), 400

    mount_point = active_mounts[device_path]['mount_point']
    candidate_path = os.path.join(mount_point, file_path.lstrip('/'))

    # Ensure the target is within the mount point
    real_mount = os.path.realpath(mount_point)
    real_target = os.path.realpath(candidate_path)
    if not (real_target == real_mount or real_target.startswith(real_mount + os.sep)):
    ...
    try:
        if os.path.isfile(real_target):
            ...
            logger.info("Deleted file: %s", real_target)
        elif os.path.isdir(real_target):
            shutil.rmtree(real_target)
            logger.info("Deleted directory: %s", real_target)
        return jsonify({'success': True})
    except Exception as e:
        logger.error("Failed to delete %s: %s", real_target, e)
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/preview')
def api_preview():
    """Preview file contents from mounted device"""
    device_path = request.args.get('device')
    file_path = request.args.get('path')
    ...
    try:
        # Get file size to determine if we should preview
        file_size = os.path.getsize(full_path)
        max_preview_size = 1024 * 1024  # 1MB limit for preview
        ...
        logger.error("Error previewing file %s: %s", full_path, e)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_delete_all():
    ...
    if not device_path:
        return jsonify({'success': False, 'error': 'Missing device'}), 400
    ...
    if not os.path.exists(real_target) or not os.path.isdir(real_target):
    ...
    try:
        ...
        for item in os.listdir(real_target):
            ...
            try:
                ...
                logger.error("Failed to delete %s: %s", item_path, e)
        
        if errors:
            return jsonify({
                'success': False,
                'error': f"Deleted {deleted_count} items, but {len(errors)} failed",
                'details': errors
            }), 500
        
        logger.info("Deleted all items in directory: %s (%d items)", real_target, deleted_count)
        return jsonify({'success': True, 'deleted_count': deleted_count})
    except Exception as e:
        logger.error("Failed to delete all in %s: %s", real_target, e)
        return jsonify({'success': False, 'error': str(e)}), 500
